...
export default function CompletePage() {
  ...
  const smsText = renderSms(operatorQ.data?.smsTemplate || DEFAULT_SMS_TEMPLATE, {
    ...
    date: formatDay(TODAY, { weekday: true }),
  });
    ...
    <main className="mx-auto flex max-w-md flex-col items-center gap-major px-section py-hero text-center">
      ...
      {info && (
        <>
          ...
          <Button shape="pill" size="lg" fullWidth onClick={() => router.push("/pos")}>{t("complete.newSale")}</Button>

          {/* Real print (opens the browser print dialog on a clean layout) + SMS. */}
          <div className="flex w-full items-center gap-tight">
            <button type="button" disabled={!info.orderId} onClick={() => router.push(`/print/tickets/${info.orderId}`)} className="flex h-12 flex-1 items-center justify-center gap-inline rounded-full border border-line bg-card text-sm text-muted active:bg-ember/10 disabled:opacity-40"><TicketIcon size={16} strokeWidth={1.5} /> {tk("printTickets")}</button>
            <button type="button" disabled={!info.orderId} onClick={() => router.push(`/print/receipt/${info.orderId}`)} className="flex h-12 flex-1 items-center justify-center gap-inline rounded-full border border-line bg-card text-sm text-muted active:bg-ember/10 disabled:opacity-40"><Printer size={16} strokeWidth={1.5} /> {tk("printReceipt")}</button>
            <button type="button" onClick={() => setSmsOpen(true)} className="flex h-12 flex-1 items-center justify-center gap-inline rounded-full border border-line bg-card text-sm text-muted active:bg-ember/10"><MessageSquare size={16} strokeWidth={1.5} /> {t("complete.sendSms")}</button>
          </div>
        </>
      )}

      {/* SMS preview — the exact message, rendered from the business template. */}
      <Modal open={smsOpen} onClose={() => setSmsOpen(false)} title={t("complete.smsTitle")} footer={<><Button shape="pill" variant="secondary" onClick={() => setSmsOpen(false)}>{t("complete.cancel")}</Button><Button shape="pill" onClick={() => { setSmsOpen(false); toast.success(t("complete.smsSent")); }}>{t("complete.sendSmsButton")}</Button></>}>
