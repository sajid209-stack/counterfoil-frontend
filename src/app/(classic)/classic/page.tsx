...
export default function PosPage() {
  ...
  /* The Schedule tab deep-links a product to open on arrival. Read ONCE, in a
     lazy initialiser, and consumed during render when the catalogue lands —
     the old file did this in an effect that setState'd, which cascades a
  ...
  const [deepLinkId] = useState<string | null>(() => {
    ...
    if (id) sessionStorage.removeItem("pos_open_product");
    return id;
  });
  ...
  /* Derived, not corrected. The old file kept a `method` state and then used
     ...
     answer is a render-time derivation: what the till WILL charge with is a
     function of what was picked and what is actually available. */
  const [pickedMethod, setMethod] = useState<PaymentMethod>("cash");
  ...
  // Gated at the source — see lib/features.
  ...
  const nonCashLive = nonCashOk;
  const method: PaymentMethod = nonCashLive || pickedMethod === "cash" ? pickedMethod : "cash";
  const deepLinked =
    ...
    <div className="grid h-full grid-cols-1 gap-tight p-tight pb-24 lg:grid-cols-[1fr_23rem] lg:pb-tight">
      ...
      <div className={`${cartOpen ? "fixed inset-x-0 bottom-0 z-50 flex max-h-[85vh] rounded-t-md pb-[env(safe-area-inset-bottom)] shadow-2xl" : "hidden"} min-h-0 flex-col border border-line bg-card lg:static lg:z-auto lg:flex lg:max-h-none lg:rounded-sm lg:pb-0 lg:shadow-none`}>
        ...
        <div className="border-t border-line p-comfortable">
          ...
          {/* Refusal guidance is not part of the coupon row — it explains why
              an extend or a discount was declined, so it must survive the
