...
export interface EventContent {
  ...
  date: string;
  startTime: string;
  /** Optional: an end time, and the day it falls on (a festival runs past
...
export function EventArchitect({
  ...
  /** Where the event's facts are edited — the wizard's Details step, the
   *  editor's Details tab. */
  onEditDetails?: () => void;
        ...
        {rows.map((id, i) => {
          ...
          return (
            <Row
              ...
              label={label}
              sample={samples.includes(id) ? t("architect.sample") : undefined}
              muted={off}
              open={open === id}
            ...
            >
              {samples.includes(id) && (
                <SampleNote onKeep={() => onEvent(id === "stats" ? { stats: event.stats } : id === "highlights" ? { highlights: event.highlights } : { info: event.info })} />
              )}

              {/* The name, the date and the cover are the event's facts, set in
                  Details; this row only says where they are drawn. */}
              {id === "hero" && <DetailsNote onEdit={onEditDetails}>{t("architect.heroFromDetails")}</DetailsNote>}
              ...
              {id === "venue" && <DetailsNote onEdit={onEditDetails}>{t("architect.venueFromDetails")}</DetailsNote>}
...
function Row({
  ...
  icon: LucideIcon;
  label: string;
  /** Set when the row still holds seeded sample content. */
  sample?: string;
  open: boolean;
  onToggle: () => void;
...
function DetailsNote({ onEdit, children }: { onEdit?: () => void; children: React.ReactNode }) {
  const t = useTranslations("events");
  return (
...
/** Seeded content is a claim about somebody's event — "16 teams, a ৳200,000
 *  prize pool" — so it is never put on sale by default. Editing it, hiding the
 *  section, or saying it stands as written are the three ways past. */
function SampleNote({ onKeep }: { onKeep: () => void }) {
  const t = useTranslations("events");
  return (
    <div className="flex flex-wrap items-center justify-between gap-tight rounded-sm bg-warning-wash px-comfortable py-tight">
      <p className="min-w-0 flex-1 text-[13px] text-fg">{t("architect.sampleNote")}</p>
      <button
        type="button"
        onClick={onKeep}
        className="flex min-h-11 items-center rounded-sm border border-line bg-card px-comfortable text-[13px] font-medium hover:border-strong md:min-h-9"
      >
        {t("architect.keepSample")}
      </button>
    </div>
  );
}
