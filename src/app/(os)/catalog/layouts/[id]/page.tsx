...
export default function SeatLayoutEditorPage() {
  ...
  if (!q.loading && (q.error || !q.data)) {
    return <PageShell title={t("editor.backToList")}><EmptyState title="Not found" action={<Button onClick={() => router.push("/catalog/layouts")}>{t("editor.backToList")}</Button>} /></PageShell>;
  }
  ...
  return (
    ...
    >
      <button type="button" onClick={() => router.push("/catalog/layouts")} className="mb-section flex items-center gap-inline text-[13px] text-muted hover:text-fg">
