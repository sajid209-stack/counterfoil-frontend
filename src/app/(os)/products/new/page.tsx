...
import { getOperator, listCategories, listLocations, listStaff } from "@/lib/api";
...
export default function NewProductPage() {
  ...
  const loading = cats.loading || locs.loading || team.loading || op.loading;
  ...
  return (
    <PageShell title="Create a product" description="A few steps — we'll handle the technical setup for you.">
      ...
      {loading ? (
        ...
        <ProductWizard categories={cats.data?.data ?? []} locations={locs.data?.data ?? []} team={team.data?.data ?? []} currency={op.data?.currency ?? "BDT"} />
      )}
    </PageShell>
  );
}
