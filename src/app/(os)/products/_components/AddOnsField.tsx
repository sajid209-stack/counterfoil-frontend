  ...
  return (
    <div className="flex flex-col gap-tight">
      <div className="flex items-center justify-between">
        <span className="type-label text-[12px] text-neutral-600">Add-ons</span>
        <Button size="sm" variant="secondary" icon={<Plus size={14} strokeWidth={1.5} />} onClick={() => onChange([...addOns, emptyAddOn()])}>Add extra</Button>
      </div>
      <p className="text-[12px] text-neutral-400">Extras offered at the counter — shoe hire, bibs, oils.</p>
      {addOns.map((a, i) => (
        <div key={i} className="grid grid-cols-1 items-end gap-tight rounded-sm border border-neutral-200 p-comfortable sm:grid-cols-[1fr_8rem_auto_auto]">
          ...
          <button type="button" aria-label="Remove" onClick={() => onChange(addOns.filter((_, idx) => idx !== i))} className="flex h-9 w-9 items-center justify-center rounded-sm border border-neutral-200 text-danger"><Trash2 size={16} strokeWidth={1.5} /></button>
        </div>
      ))}
    </div>
  );
